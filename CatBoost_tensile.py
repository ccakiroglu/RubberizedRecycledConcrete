...
colnamesT=[r'$w/c$', r'$NCA\text{ }FM$', r'$NCA \text{ }[kg/m^3]$', \
       ...
       r'$Steel\text{ }Tire\text{ }Wires$',r'$Age$', r'$f_t$']

Tdir='G:\\My Drive\\Papers\\2023\\RubberizedConcrete\\EXCELCSV\\Tensile2.csv'
Eqndir='G:\\My Drive\\Papers\\2023\\RubberizedConcrete\\EXCELCSV\\TensileEqn.csv'
df = read_csv(Tdir,header=0,names=colnamesT)
data=df.values
X, y = data[:, :-1], data[:, -1]
...
print('Explained variance score (EVS):',explained_variance_score(y_train, yhat_train));
...
print('Explained variance score (EVS):',explained_variance_score(y_test, yhat_test));
...
ax.scatter(yhat_train,y_train, marker='o',facecolor='teal',edgecolor='teal', label=r'$CatBoost\text{ }train$')
ax.scatter(yhat_test,y_test, marker='s',facecolor='gold',edgecolor='gold', label=r'$CatBoost\text{ }test$')
xk=[0,10];yk=[0,10];ykPlus10Perc=[0,11];ykMinus10Perc=[0,9];
...
ax.grid(True)
ax.set_xticks(array([0,2,4,6,8,10]))
ax.set_xlabel(r'$f_{t,predicted}\hspace{0.5em}[MPa]$', fontsize=17)
ax.set_ylabel(r'$f_{t,test}\hspace{0.5em}[MPa]$', fontsize=17)
